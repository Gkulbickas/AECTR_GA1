...
ll_garch_ml <- function(par, data){
    ...
    # Check constraints
    if (beta < 0 || alpha <= abs(delta) || gamma <= 0 || omega <= 0 || v <= 2) {
        return(1e10)
    }
...
# Test using Table 1 optimal parameters for M3 APPL
test_opt_par <- c(mu = 0.108, lambda = 0.022, omega = 0.012, alpha = 0.073, beta = 0.915, delta = 0.071, gamma = 0.439, v = 4.402)
...
print("TEST RUN: Ticker, Estimated Parameters: mu, lambda, omega, alpha, beta, delta, gamma, v, LogL, AIC, BIC")
cat("APPL", round(test_est$par, 3), round(-test_est$value, 0), round(test_aic_bic[1], 0), round(test_aic_bic[2], 0), "\n", "\n")

print("Ticker, Estimated Parameters: mu, lambda, omega, alpha, beta, delta, gamma, v, LogL, AIC, BIC")
...
for(ticker in names(asset_rets)){
    ...
    cat(ticker, round(est_garch_ml$par, 3), round(-est_garch_ml$value, 0), round(aic_bic_garch_ml[1], 0), round(aic_bic_garch_ml[2], 0), "\n")
}
